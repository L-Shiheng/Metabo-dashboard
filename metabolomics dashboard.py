...
with st.sidebar:
    ...
    if len(grps) < 2: st.error("错误：组别 < 2"); st.stop()
    c1, c2 = st.columns(2)
...
st.success(f"✅ 数据清洗完成：已处理 {raw_df.shape[0]} 个样本，保留 {len(feats)} 个特征 (已执行 Log2 转换)")
...
with tabs[2]:
    ...
    top_vip = vip_df.sort_values('VIP', ascending=True).tail(25) # 取前25
    
    col1, col2, col3 = st.columns([1, 3, 1]) # 让图表宽一点
    with col2:
        # 使用 Bar chart
        fig_vip = px.bar(top_vip, x="VIP", y="Metabolite", orientation='h',
                         color="VIP", color_continuous_scale="RdBu_r",
                         width=800, height=700) # 增加宽度和高度
        ...
        # 布局优化：增加左边距 (Left Margin) 防止文字挤压
        fig_vip.update_layout(
            template="simple_white",
            width=800, height=700,
            title={'text': "Variable Importance in Projection (VIP)", 'x':0.5, 'xanchor': 'center', 'font': dict(size=20, family="Arial, bold")},
            ...
            # 关键：设置 margin.l 为较大值 (如 200)，给长名称留出空间
...
with tabs[3]:
    ...
    with col2:
        fig_vol = px.scatter(res_stats, x="Log2_FC", y="-Log10_P", color="Sig",
                             ...
                             hover_data=["Metabolite", "P_Value", "FDR"], # 悬停显示名字
...
# --- Tab 5: 热图 ---
with tabs[4]:
    if len(sig_metabolites) < 2: st.warning("显著差异物 < 2，无法绘制热图")
    else:
...
with tabs[5]:
    ...
    with c2:
        ...
        if target_feat:
            ...
            # pointpos=0 将散点放在箱子中间，jitter=0.5 增加散开程度
